    ...
    int64_t causal)
{
    ...
    int64_t ns;
    {
        const char* ove = getenv("GQA_NS_OVERRIDE");
        const char* ovb = getenv("GQA_NS_BATCH");   // batch matching for override
        const char* ovk = getenv("GQA_NS_KV");      // kv matching for override
        int64_t ovb_val = ovb ? atoll(ovb) : -1;
        int64_t ovk_val = ovk ? atoll(ovk) : -1;
        if (ove && (ovb_val < 0 || ovb_val == batch_size) && (ovk_val < 0 || ovk_val == num_heads_k)) {
            ns = atoll(ove);
            if (ns < 1) ns = 1;
        } else
    if (pages <= 4) {
        ...
        // case14 (batch1,kv4,pages3845): ns~160 optimum.
        // case13 (batch1,kv8,pages3686): ns~90 optimum (weakest case, was 64).
        if (num_heads_k == 8) {
            ns = (pages <= 1024) ? 64 : 90;   // batch1 kv8
        } else {
            ns = (pages <= 1024) ? 64 : 160;  // batch1 kv4
        }
    } else {
