    ...
    int tokens_per_split)
{
    ...
    // H1 full/tail split. tokens_per_split is a multiple of PAGE, so within a
    // split every page before the last holds 16 valid tokens. nfull pages are
    // swept by an UNMASKED steady-state loop (no ntok clamp, no padding-mask
    // predicates, no -INF sentinel handling); the single final partial page of
    // the sequence is handled once, separately, with the exact masked path.
    ...
    for (int i = 0; i < nfull; i++) {
        ...
        // (no padding mask needed: all 16 tokens valid)
        ...
        // merged-max softmax: one rescale (alpha) on the running state, and
        // the page weights are computed relative to the MERGED max (no beta).
        ...
        // PV: A = P frag (relative to merged max), B = V_T frag
    ...
    if (rem != 0) {
        ...
        // masked path: only the first ntok tokens of this page are valid
        __nv_bfloat16 (*Kb)[HEAD+2] = K_b;
        __nv_bfloat16 (*Vt)[PAGE+2] = V_T;
        ...
        // merged-max softmax (same as steady state)
        float p[4];
        float ls;
        float mnew = fmaxf(m, mx);
        float alpha = __expf(m - mnew);
