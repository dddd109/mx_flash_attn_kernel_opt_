/*
 * Paged GQA decode kernel for MetaX C500 (optimized).
 * Uses compiler builtin __builtin_mxc_mma_16x16x16bf16.
 * V token-major + VPAD=8 (no per-page transpose). Merged-max softmax.
 * Split policy tuned for real OJ case table (incl. batch=1 kv=8 long seq -> ns=64).
 */
#include <stdint.h>
#include <stdlib.h>
#include <cuda_bf16.h>
    ...
    int64_t causal)
{
    ...
    if (pages <= 4) {
        ...
        // batch==1: gqa<=8. For very long single-batch (pages>~2000), ns=128
        // over-splits (combine overhead); 64 is the measured optimum.
        ns = (pages <= 1024) ? 64 : ((batch_size == 1 && num_heads_k == 8 && pages > 2000) ? 64 : 128);
    } else {
