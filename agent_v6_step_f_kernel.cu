    ...
    int64_t causal)
{
    ...
    {
        const char* ov = getenv("GQA_NS");
        if (ov) {
            int nsv = atoi(ov);
            tokens_per_split = nsv == 1 ? (int)seqlen_k : (int)((seqlen_k + nsv - 1) / nsv);
            tokens_per_split = ((tokens_per_split + 15) / 16) * 16;
            ...
            // tokens_per_split: fewer work units (small batch) -> each block should
            // do a bigger chunk; many work units -> small chunks keep SMs busy.
            // For the medium-batch bucket, longer seqlen benefits from more splits.
            int64_t work_units = batch_size * num_heads_k;
            int64_t ns_target;
            if (work_units <= 8)      ns_target = 128;
            else if (work_units <= 32) ns_target = 96;
            else if (work_units <= 64) ns_target = (seqlen_k > 8000) ? 80 : 48;
            else                       ns_target = 13;
            int64_t tps = (int64_t)((double)seqlen_k / (double)ns_target);
            if (tps < 48) tps = 48;
            if (tps > 480) tps = 480;
            tokens_per_split = (int)(((tps + 15) / 16) * 16);
            ns = (int)((seqlen_k + tokens_per_split - 1) / tokens_per_split);
            if (ns < 1) ns = 1;
        }
    }
