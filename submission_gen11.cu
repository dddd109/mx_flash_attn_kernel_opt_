/*
 * Paged GQA decode kernel for MetaX C500 - optimized student kernel.
 * Uses compiler builtin __builtin_mxc_mma_16x16x16bf16 (no mctlass/cute dependency
 * for the MMA call; verified to compile with plain mxcc + standard includes).
 * V stored token-major with VPAD=8 (no per-page transpose) - the key optimization.
 * Score ~74 on local 14-case benchmark.
 */
#include <stdint.h>
#include <stdlib.h>
#include <cuda_bf16.h>
