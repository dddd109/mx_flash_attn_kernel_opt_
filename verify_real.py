"""14-case correctness vs flash_attn using the SPJ-CONFIRMED case table.
Usage: python3 verify_real.py <lib.so>
Edge cases 1-3 require match 1.0; perf cases >=0.99; no 8x outliers.
"""
...
for c, (batch, seq, kv) in CASES.items():
    ...
    ref = flash_attn_with_kvcache(q, k, v, None, None, cache_seqlens=cs,
    ...
    req = 1.0 if c <= 3 else 0.99
    ok = m >= req and not o
    if not ok: allok = False
    print(f"case {c:>2} (b{batch:>3} seq{seq:>6} kv{kv}): match={m:.4f} outlier={o} {'OK' if ok else 'FAIL'}")
